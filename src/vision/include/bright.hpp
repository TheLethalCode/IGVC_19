...
Mat brightest(Mat img) //brightest pixels below brightestPixelThreshold will be neglected (reduces noise)
{
	cout << "starting brightest pixel" << endl;
	...
	for (int i = 0; i < Simg2.rows; i++)
	{
		...
		if (max > brightestPixelThreshold)
	...
	Simg4.copyTo(img);
	cout << "exiting brightest pixel" << endl;
	return img;
}
